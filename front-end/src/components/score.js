...
const Score = ({ uploaded, uploading, callback }) => {
    ...
    useEffect(() => {
        if (isLoading) {
            ...
            getPrediction(coords.x, coords.y, coords.z, onDownloadProgress, showMaps).then((res) => {
                console.log(res);
    ...
    const handleClick = () => {
        if (window.papayaContainers && window.papayaContainers.length > 0) {
            ...
            visitedCoords.forEach((visitedCoord) => {
                coordsExist = JSON.stringify(visitedCoord) === JSON.stringify(computedCoords);
            });
            if (!coordsExist) {
                setScore(null);
                        ...
                        <Card.Body>
                            ...
                            <ListGroup>
                                {visitedCoords.slice().reverse().map((coordinates, idx) => (
                                    <ListGroup.Item key={idx} action onClick={() => window.papayaContainers[0].viewer.gotoCoordinate(coordinates)}>
                                        <div className="d-flex justify-content-between">
                                            <Badge variant="info" className="d-flex align-items-center">Coords {visitedCoords.length - idx}</Badge>
                                            <span data-testid="X">X: {coordinates.x}</span>
                                            <span data-testid="Y">Y: {coordinates.y}</span>
                                            <span data-testid="Z">Z: {coordinates.z}</span>
                                        </div>
                                    </ListGroup.Item>
                                ))}
                            </ListGroup>
